...
function loadSketchPanel(id)
{
  ...
  panel2.hideEvent.subscribe(function() {
    setTimeout(function() {stopApplet("applet"+id);},100);
    panel2.dispose();
  });
...
'<applet id="applet'+id+'" name="PApplet" code="${internal.launcherClass}" ' +
 'codebase="${lib}" archive="${internal.allJars}" width="${applet.width}" height="${applet.height}" mayscript="true">' +
...
function appletObject(container_id,id,propsRef)
{
  var ao = new AppletObject(
	...
	'${project.name}', // The ID attribute of the created tag... so you can access the DOM node later on if needed!
	'applet'+id
  );
